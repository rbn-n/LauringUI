...
local INFO = module:RegisterInfobar("Friend", Config.Infobar.FriendsPosition)
...
function INFO:FriendsPanel_Init()
	...
	infoFrame = CreateFrame("Frame", "LauringUIFriendsFrame", INFO)
	...
	infoFrame:SetScript("OnLeave", function(frame)
		frame:SetScript("OnUpdate", IsPanelCanHide)
	end)

	Core.CreateFS(infoFrame, 16, "|cff0099ff"..FRIENDS_LIST, nil, "TOPLEFT", 15, -10)
	infoFrame.friendCountText = Core.CreateFS(infoFrame, 14, "-/-", nil, "TOPRIGHT", -15, -12)
	infoFrame.friendCountText:SetTextColor(0, .6, 1)

	local scrollFrame = CreateFrame("ScrollFrame", "LauringUIFriendsInfobarScrollFrame", infoFrame, "HybridScrollFrameTemplate")
	...
	Core.CreateFS(infoFrame, 13, DB.LineString, false, "BOTTOMRIGHT", -12, 42)
	local whspInfo = DB.InfoColor..DB.RightButton..L["Whisper"]
	Core.CreateFS(infoFrame, 13, whspInfo, false, "BOTTOMRIGHT", -15, 26)
	local invtInfo = DB.InfoColor.."ALT +"..DB.LeftButton..L["Invite"]
	Core.CreateFS(infoFrame, 13, invtInfo, false, "BOTTOMRIGHT", -15, 10)
end
...
function INFO:FriendsPanel_Update()
	local scrollFrame = LauringUIFriendsInfobarScrollFrame
	local usedHeight = 0
...
INFO.onEnter = function(self)
	...
	if LauringUIGuildInfobar and LauringUIGuildInfobar:IsShown() then
		LauringUIGuildInfobar:Hide()
	end
