...
local info = module:RegisterInfobar("Dura", Config.Infobar.DurabilityPosition)
...
info.onEvent = function(self, event)
	...
	if UpdateAllSlots() > 0 then
		...
		self.text:SetFormattedText("%s%%|r"..L["D"], Core.HexRGB(r, g, b)..floor(localSlots[1][3]*100))
	else
...
info.onEnter = function(self)
	...
	for i = 1, #localSlots do
		if localSlots[i][3] ~= 1000 then
			...
			totalCost = totalCost + select(3, Core.ScanTip:SetInventoryItem("player", slot))
		end
	end
	...
	GameTooltip:AddDoubleLine(" ", DB.ScrollButton..L["Auto Repair"]..": "..repairlist[LauringUIAccountDB["RepairType"]].." ", 1,1,1, .6,.8,1)
	GameTooltip:Show()
end
...
function autoRepair(override)
	...
	if canRepair and repairAllCost > 0 then
		if (not override) and LauringUIAccountDB["RepairType"] == 1 and IsInGuild() and CanGuildBankRepair() and GetGuildBankWithdrawMoney() >= repairAllCost then
			RepairAllItems(true)
		else
