...
local module = Core:GetModule("Chat")
...
local function ReplaceMessage(msg, r, g, b)
	local hexRGB = Core.HexRGB(r, g, b)
...
function module:ChatCopy_OnClick(btn)
	...
	elseif btn == "RightButton" then
		Core:TogglePanel(menu)
		Config.DB["Chat"]["ShowMenu"] = menu:IsShown()
	end
end

local function ResetChatAlertJustify(chatAlert)
	chatAlert:SetJustification("LEFT")
end
...
function module:ChatCopy_CreateMenu()
	...
	menu:SetShown(Config.DB["Chat"]["ShowMenu"])
...
function module:ChatCopy_Create()
	frame = CreateFrame("Frame", "LauringUIChatCopy", UIParent)
	...
	copy:SetPoint("TOPRIGHT", _G.ChatFrame1, -1, 0)
	copy:SetSize(25, 25)
	copy:SetAlpha(.5)
	copy.Icon = copy:CreateTexture(nil, "ARTWORK")
	copy.Icon:SetAllPoints()
	copy.Icon:SetTexture(DB.CopyTexture)
	...
	Core.AddTooltip(copy, "ANCHOR_RIGHT", copyStr)
	copy:HookScript("OnEnter", function() copy:SetAlpha(1) end)
	copy:HookScript("OnLeave", function() copy:SetAlpha(.5) end)

	Core.ReskinClose(frame.close)
	Core.ReskinScroll(ChatCopyScrollFrameScrollBar)
end
