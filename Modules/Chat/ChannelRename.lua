...
local CHAT_TIMESTAMP_FORMAT = CHAT_TIMESTAMP_FORMAT
...
function module:UpdateChannelNames(text, ...)
	...
	if LauringUIAccountDB["TimestampFormat"] > 1 then
		...
		local timeStamp = BetterDate(DB.GreyColor..timestampFormat[LauringUIAccountDB["TimestampFormat"]].."|r", realmTime or locTime)
		text = timeStamp..text
	end
