...
function Tooltip:UpdateItemSellPrice()
	local frame = Tooltip:GetMouseFocus()
...
function Tooltip:AddLineForID(id, linkType, noadd)
	...
	if linkType == types.item then
		Tooltip.UpdateItemSellPrice(self)
	end
...
function Tooltip:UpdateSpellCaster(...)
	...
	if unitCaster then
		...
		local hexColor = Core.HexRGB(Core.UnitColor(unitCaster))
