...
local module = Core:GetModule("Bags")
...
local function isItemJunk(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterJunk"] then return end
	return (item.quality == LE_ITEM_QUALITY_POOR or LauringUIAccountDB["CustomJunkList"][item.id]) and item.hasPrice
end

local function isItemEquipSet(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterEquipSet"] then return end
	return item.isInSet
end

local function isItemAmmo(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterAmmo"] then return end

	if Config.DB["Bags"]["GatherEmpty"] and not item.texture then
		return false
	end
...
local function isItemEquipment(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterEquipment"] then return end
...
local function isItemConsumable(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterConsumable"] then return end
...
local function isItemLegendary(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterLegendary"] then return end
...
local function isItemCollection(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterCollection"] then return end
	return item.id and C_ToyBox_GetToyInfo(item.id) or isMountOrPet(item)
end

local function isItemCustom(item, index)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterFavourite"] then return end
	local customIndex = item.id and Config.DB["Bags"]["CustomItems"][item.id]
	return customIndex and customIndex == index
end

local function isEmptySlot(item)
	if not Config.DB["Bags"]["GatherEmpty"] then return end
...
local function isQuestItem(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterQuest"] then return end
	return item.isQuestItem
end

local function isItemBOE(item)
	if not Config.DB["Bags"]["ItemFilter"] then return end
	if not Config.DB["Bags"]["FilterBOE"] then return end
