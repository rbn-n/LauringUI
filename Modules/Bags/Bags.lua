...
local module = Core:RegisterModule("Bags")
...
function module:UpdateBagsAnchor(parent, bags)
	...
	local index = 1
	local perRow = Config.DB["Bags"]["BagsPerRow"]
...
function module:UpdateBankAnchor(parent, bags)
	...
	local index = 1
	local perRow = Config.DB["Bags"]["BankPerRow"]
	anchorCache[index] = parent
...
function module:CreateInfoFrame()
	...
	local bg = Core.CreateBDFrame(search, 0, true)
	bg:SetPoint("TOPLEFT", -5, -5)
	bg:SetPoint("BOTTOMRIGHT", 5, 5)
	search.textFilters = BagSmartFilter

	infoFrame.title = SEARCH
	Core.AddTooltip(infoFrame, "ANCHOR_TOPLEFT", DB.InfoColor..L["BagSearchTip"])
end

local function ToggleWidgetButtons(self)
	Config.DB["Bags"]["HideWidgets"] = not Config.DB["Bags"]["HideWidgets"]

	local buttons = self.__owner.widgetButtons

	for index, button in pairs(buttons) do
		if index > 2 then
			button:SetShown(not Config.DB["Bags"]["HideWidgets"])
		end
	end

	if Config.DB["Bags"]["HideWidgets"] then
		self:SetPoint("RIGHT", buttons[2], "LEFT", -1, 0)
		Core.SetupArrow(self.__texture, "left")
		self.tag:Show()
	else
		self:SetPoint("RIGHT", buttons[#buttons], "LEFT", -1, 0)
		Core.SetupArrow(self.__texture, "right")
		self.tag:Hide()
	end
	self:Show()
end
...
function module:CreateCollapseArrow()
	...
	tex:SetAllPoints()
	Core.SetupArrow(tex, "right")
	bu.__texture = tex
	bu:SetScript("OnEnter", Core.Texture_OnEnter)
	bu:SetScript("OnLeave", Core.Texture_OnLeave)
	...
	bu.__owner = self
	Config.DB["Bags"]["HideWidgets"] = not Config.DB["Bags"]["HideWidgets"] -- reset before toggle
...
local function CloseOrRestoreBags(self, btn)
	if btn == "RightButton" then
		...
		Config.DB["TempAnchor"][bag:GetName()] = nil
		Config.DB["TempAnchor"][bank:GetName()] = nil
		bag:ClearAllPoints()
...
function module:CreateCloseButton(f)
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\RAIDFRAME\\ReadyCheck-NotReady")
	bu:RegisterForClicks("AnyUp")
	bu.__owner = f
	bu:SetScript("OnClick", CloseOrRestoreBags)
	bu.title = CLOSE.."/"..RESET
	Core.AddTooltip(bu, "ANCHOR_TOP")
...
local function ToggleBackpacks(self)
	...
	Core:TogglePanel(parent.BagBar)
...
function module:CreateBagToggle()
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\Buttons\\Button-Backpack-Up")
	bu.__owner = self
...
function module:CreateKeyToggle()
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\ICONS\\INV_Misc_Key_12")
...
function module:CreateFreeSlots()
	...
	slot:SetHighlightTexture(DB.BackgroundTexture)
	slot:GetHighlightTexture():SetVertexColor(1, 1, 1, .25)
	Core:SetInside(slot:GetHighlightTexture())
	Core.CreateBD(slot, .3)
	...
	Core.AddTooltip(slot, "ANCHOR_RIGHT", L["FreeSlots"])
	slot.__owner = self

	local tag = self:SpawnPlugin("TagDisplay", "[space]", slot)
	Core.SetFontSize(tag, Config.DB["Bags"]["FontSize"] + 2)
...
local function saveSplitCount(self)
	...
	Config.DB["Bags"]["SplitCount"] = tonumber(count) or 1
end
...
function module:CreateSplitButton()
	...
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\HELPFRAME\\ReportLagIcon-AuctionHouse")
	...
	bu:SetScript("OnClick", function(self)
		...
		if splitEnable then
			...
			splitFrame:Show()
			editbox:SetText(Config.DB["Bags"]["SplitCount"])
		else
			self.__turnOff()
		end
...
local function splitOnClick(self)
	...
	if texture and not locked and itemCount and itemCount > Config.DB["Bags"]["SplitCount"] then
		SplitContainerItem(self.bagId, self.slotId, Config.DB["Bags"]["SplitCount"])
...
StaticPopupDialogs["LAURINGUI_RENAMECUSTOMGROUP"] = {
	...
	OnAccept = function(self)
		...
		Config.DB["Bags"]["CustomNames"][index] = text ~= "" and text or nil
...
function module:MoveItemToCustomBag(index)
	...
	if index == 0 then
		if Config.DB["Bags"]["CustomItems"][itemID] then
			Config.DB["Bags"]["CustomItems"][itemID] = nil
		end
	else
		Config.DB["Bags"]["CustomItems"][itemID] = index
	end
...
function module:IsItemInCustomBag()
	...
	local itemID = module.selectItemID
	return (index == 0 and not Config.DB["Bags"]["CustomItems"][itemID]) or (Config.DB["Bags"]["CustomItems"][itemID] == index)
end
...
function module:CreateFavouriteButton()
	...
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\Common\\friendship-heart")
...
local function favouriteOnClick(self)
	...
	if texture and quality > LE_ITEM_QUALITY_POOR then
		...
		EasyMenu(module.CustomMenu, Core.EasyMenu, self, 0, 0, "MENU")
	end
end
...
StaticPopupDialogs["LAURINGUI_WIPE_JUNK_LIST"] = {
	...
	button2 = NO,
	OnAccept = function()
		wipe(LauringUIAccountDB["CustomJunkList"])
	end,
	whileDead = 1,
}
...
function module:CreateJunkButton()
	...
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\BUTTONS\\UI-GroupLoot-Coin-Up")
	bu.Icon:SetPoint("TOPLEFT", Config.PixelMultiplexer, -3)
	bu.Icon:SetPoint("BOTTOMRIGHT", -Config.PixelMultiplexer, -3)
	bu.__turnOff = function()
		Core.SetBorderColor(bu.bg)
		bu.text = nil
...
local function customJunkOnClick(self)
	...
	if texture and price > 0 then
		if LauringUIAccountDB["CustomJunkList"][itemID] then
			LauringUIAccountDB["CustomJunkList"][itemID] = nil
		else
			LauringUIAccountDB["CustomJunkList"][itemID] = true
		end
		ClearCursor()
		module:UpdateAllBags()
	end
...
function module:CreateDeleteButton()
	...
	local bu = Core.CreateButton(self, 22, 22, true, "Interface\\Buttons\\UI-GroupLoot-Pass-Up")
...
function module:OnLogin()
	if not Config.DB["Bags"]["Enable"] then return end

	-- Settings
	local iconSize = Config.DB["Bags"]["IconSize"]
	local showNewItem = Config.DB["Bags"]["ShowNewItem"]
	...
	function Backpack:OnInit()
		...
		for bagType, groups in pairs(module.ContainerGroups) do
			for _, container in ipairs(groups) do
				...
				Core.CreateMF(container, parent, true)
			end
		end
	end
	...
	function MyButton:OnCreate()
		...
		Core:SetInside(self:GetHighlightTexture())
		self:SetSize(iconSize, iconSize)

		Core:SetInside(self.Icon)
		self.Icon:SetTexCoord(unpack(DB.TexCoord))
		self.Count:SetPoint("BOTTOMRIGHT", -1, 2)
		Core.SetFontSize(self.Count, Config.DB["Bags"]["FontSize"])
		Core:SetInside(self.Cooldown)

		Core.CreateBD(self, .3)
		...
		self.Quest = Core.CreateFS(self, 30, "!", "system", "LEFT", 3, 0)
		self.iLvl = Core.CreateFS(self, Config.DB["Bags"]["FontSize"], "", false, "BOTTOMLEFT", 1, 2)

		if showNewItem then
			self.glowFrame = Core.CreateGlowFrame(self, iconSize)
		end
	...
	function MyButton:OnUpdateButton(item)
		if self.JunkIcon then
			if (MerchantFrame:IsShown() or customJunkEnable) and (item.quality == LE_ITEM_QUALITY_POOR or LauringUIAccountDB["CustomJunkList"][item.id]) and item.hasPrice then
				self.JunkIcon:Show()
			else
				self.JunkIcon:Hide()
			end
		...
		if Config.DB["Bags"]["CustomItems"][item.id] and not Config.DB["Bags"]["ItemFilter"] then
			self.Favourite:Show()
		else
			self.Favourite:Hide()
		end
		...
		self.iLvl:SetText("")
		if Config.DB["Bags"]["BagsiLvl"] and isItemNeedsLevel(item) then
			local level = item.level
			if level and level > Config.DB["Bags"]["iLvlToShow"] then
		...
		if Config.DB["Bags"]["SpecialBagsColor"] then
	...
	function module:GetContainerColumns(bagType)
		if bagType == "Bag" then
			return Config.DB["Bags"]["BagsWidth"]
		elseif bagType == "Bank" then
			return Config.DB["Bags"]["BankWidth"]
		end
	end
	...
	function MyContainer:OnContentsChanged(gridOnly)
		...
		if self.freeSlot then
			if Config.DB["Bags"]["GatherEmpty"] and (self.bagGroup == 0 or (self.totalFree > 0 and Config.DB["Bags"]["ItemFilter"])) then
	...
	function MyContainer:OnCreate(name, settings)
		...
		if label then
			self.label = Core.CreateFS(self, 14, label, true, "TOPLEFT", 5, -8)
			return
		end
		...
		self:HookScript("OnShow", Core.RestoreMF)
	end
	...
	local function updateBagSize(button)
		...
		Core.SetFontSize(button.Count, Config.DB["Bags"]["FontSize"])
		Core.SetFontSize(button.iLvl, Config.DB["Bags"]["FontSize"])
	end

	function module:UpdateBagSize()
		iconSize = Config.DB["Bags"]["IconSize"]
		for _, container in pairs(Backpack.contByName) do
			...
			if container.freeSlot then
				...
				Core.SetFontSize(container.freeSlot.tag, Config.DB["Bags"]["FontSize"]+2)
			end
	...
	-- Init
	Config.DB["Bags"]["GatherEmpty"] = not Config.DB["Bags"]["GatherEmpty"]
	ToggleAllBags()
	Config.DB["Bags"]["GatherEmpty"] = not Config.DB["Bags"]["GatherEmpty"]
	ToggleAllBags()
	module.initComplete = true

	Core:RegisterEvent("TRADE_SHOW", module.OpenBags)
	--Core:RegisterEvent("TRADE_CLOSED", module.CloseBags)
	Core:RegisterEvent("AUCTION_HOUSE_SHOW", module.OpenBags)
		...
		for _, info in pairs(INFO.modules) do
			if info.name == "Bags" then
				Backpack.OnOpen = function()
					if not LauringUIAccountDB["ShowSlots"] then return end
					info:onEvent()
				end
				break
			end
