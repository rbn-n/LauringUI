...
local Loot = Core:GetModule("Loot")
-------------------------
...
local function NameGenerator(name)
    local index = 0
    return function()
        index = index + 1
        return name .. index
...
local fontSize = 14
local parentFrame
local GenerateName = NameGenerator(addonName.."_LootRoll")
...
local function SetTip(button)
	...
	if rolls then
		for _, rollerInfo in next, rolls do
			...
			local r, g, b = Core.ClassColor(className)
...
local function StatusUpdate(button, elapsed)
	...
	if button.elapsed and button.elapsed > 0.1 then
		...
		if timeLeft <= 0 then -- workaround for other addons auto-passing loot
			Loot.LootRoll_Cancel(bar, nil, bar.rollID)
		else
...
local function CreateRollButton(parent, texture, rolltype, tiptext, points, atlas)
	...
	button:SetWidth(Config.DB["Loot"]["Height"] - 4)
	button:SetHeight(Config.DB["Loot"]["Height"] - 4)
...
function Loot:CreateRollBar(name)
	local bar = CreateFrame("Frame", name or GenerateName(), UIParent)
	bar:SetSize(Config.DB["Loot"]["Width"], Config.DB["Loot"]["Height"])
	bar:SetFrameStrata("MEDIUM")
	bar:SetFrameLevel(10)
	bar:SetScript("OnEvent", Loot.LootRoll_Cancel)
	bar:RegisterEvent("CANCEL_LOOT_ROLL")
	bar:Hide()

	local button = CreateFrame("Button", nil, bar)
	button:SetPoint("RIGHT", bar, "LEFT", - (Config.PixelMultiplexer*2), 0)
	button:SetSize(bar:GetHeight() - (Config.PixelMultiplexer*2), bar:GetHeight() - (Config.PixelMultiplexer*2))
	...
	button.bg = Core.SetBD(button.icon)
	...
	status:SetPoint("TOPLEFT", Config.PixelMultiplexer, -(Config.DB["Loot"]["Style"] == 2 and bar:GetHeight() / 1.6 or Config.PixelMultiplexer))
...
local function GetFrame()
	for _, bar in next, Loot.RollBars do
		if not bar.rollID then
			return bar
		end
	end

	local bar = Loot:CreateRollBar()
	if next(Loot.RollBars) then
		if Config.DB["Loot"]["Direction"] == 2 then
			bar:SetPoint("TOP", Loot.RollBars[#Loot.RollBars], "BOTTOM", 0, -4)
		else
			bar:SetPoint("BOTTOM", Loot.RollBars[#Loot.RollBars], "TOP", 0, 4)
		end
	else
		bar:SetPoint("TOP", parentFrame, "TOP")
	end

	tinsert(Loot.RollBars, bar)
	return bar
end
...
function Loot:LootRoll_Start(rollID, rollTime)
	...
	if not name then
		for _, rollBar in next, Loot.RollBars do
			if rollBar.rollID == rollID then
				Loot.LootRoll_Cancel(rollBar, nil, rollID)
			end
		end
	...
	bar.button.ilvl:SetText(Config.DB["Loot"]["ItemLevel"] and level or "")
...
function Loot:LootRoll_Update(itemIdx, playerIdx)
...
function Loot:LootRoll_Cancel(_, rollID)
...
function Loot:OnLogin()
	...
	parentFrame:SetSize(Config.DB["Loot"]["Width"], Config.DB["Loot"]["Height"])
	Core.Mover(parentFrame, L["teksLoot LootRoll"], "teksLoot", {"TOP", UIParent, 0, -200})
	fontSize = Config.DB["Loot"]["Height"] / 2

	Core:RegisterEvent("LOOT_HISTORY_ROLL_CHANGED", self.LootRoll_Update)
	Core:RegisterEvent("START_LOOT_ROLL", self.LootRoll_Start)
...
function Loot:LootRollTest()
	...
	testFrame = Loot:CreateRollBar("LauringUI_LootRoll")
...
function Loot:UpdateLootRollTest()
	if not parentFrame then return end
	...
	local width, height = Config.DB["Loot"]["Width"], Config.DB["Loot"]["Height"]
	testFrame:Show()
	testFrame:SetSize(width, height)
	testFrame.button:SetSize(height - (Config.PixelMultiplexer*2), height - (Config.PixelMultiplexer*2))
	...
	testFrame.status:SetPoint("TOPLEFT", Config.PixelMultiplexer, -(Config.DB["Loot"]["Style"] == 2 and testFrame:GetHeight() / 1.6 or Config.PixelMultiplexer))
...
SlashCmdList["LAURINGUI_TEKS"] = function()
	Loot:LootRollTest()
end
SLASH_LAURINGUI_TEKS1 = "/teks"