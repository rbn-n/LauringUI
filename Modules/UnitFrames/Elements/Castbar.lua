...
local function UpdateSpellTarget(self, unit)
	...
	if unitTarget and UnitExists(unitTarget) then
		...
		if UnitIsUnit(unitTarget, "player") then
			...
			nameString = Core.HexRGB(Core.UnitColor(unitTarget))..UnitName(unitTarget)
		end
...
local function UpdateCastBarColor(self, unit)
	local color = Config.DB["Castbars"]["CastingColor"]

	if not UnitIsUnit(unit, "player") and self.notInterruptible then
		color = Config.DB["Castbars"]["NotInterruptColor"]
	end

	self:SetStatusBarColor(color.r, color.g, color.b)
end

local function CreateAndUpdateBarTicks(bar, numTicks)
	for i = 1, #ticks do
		ticks[i]:Hide()
	end
	...
	if numTicks and numTicks > 0 then
		...
		for i = 1, numTicks-1 do
			if not ticks[i] then
				...
				ticks[i]:SetTexture(DB.StatusBarTexture2)
				ticks[i]:SetVertexColor(0, 0, 0, .7)
				ticks[i]:SetWidth(Config.PixelMultiplexer)
...
function UF:PostCastStart(unit)
	...
	elseif unit == "player" then
		...
		CreateAndUpdateBarTicks(self, numTicks)
	end
...
function UF:ToggleCastBar(unit)
    ...
    local showCastBar = Config.DB["Castbars"]["Show"..unit]

	if showCastBar and not self:IsElementEnabled("Castbar") then
		self:EnableElement("Castbar")
	elseif not showCastBar and self:IsElementEnabled("Castbar") then
...
local function CreateSpark(castbar)
	...
	castbar.Spark:SetPoint("TOPLEFT", castbar:GetStatusBarTexture(), "TOPRIGHT", -10, 10)
...
local function CreateSafeZone(castbar, frame)
	local safeZone = castbar:CreateTexture(nil, "OVERLAY")
	safeZone:SetTexture(DB.StatusBarTexture2)
	...
	UF:ToggleCastBarLatency(frame)
end

function UF:ToggleCastBarLatency(frame)
	frame = frame or oUF_Player
...
function UF:CreateCastbar(frame)
	...
	castbar.PostCastDelayed = UF.PostCastUpdate
	castbar.PostChannelUpdate = UF.PostCastUpdate
	castbar.PostCastFailed = UF.PostCastFailed
	castbar.PostCastInterruptible = UF.PostUpdateInterruptible
