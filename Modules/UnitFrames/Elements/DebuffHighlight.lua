...
function UF:CheckForDispellableAura(frame, unit)
	...
	if foundType then
		local color = dispellableTypes[foundType]
			or DB.DebuffHighlightColors[foundType]
			or { r = 0.3, g = 0.3, b = 1 }

		frame.DebuffHighlight:SetVertexColor(
			color.r,
			color.g,
			color.b,
			frame.DebuffHighlightAlpha or 0.5
		)
	else
...
function UF:CreateDebuffHighlight(frame, registerUnitAuraEvent)
	...
	Core:RegisterEvent("PLAYER_TALENT_UPDATE", function()
