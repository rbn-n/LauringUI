...
function UF:UpdateRaidBuffsWhite()
	...
	for spellID in pairs(Config.RaidBuffsWhite) do
		local name = GetSpellInfo(spellID)
		if name then
			if LauringUIAccountDB["RaidBuffsWhite"][spellID] == nil then
				UF.RaidBuffsWhite[spellID] = true
			end
		end
	end

	for spellID, value in pairs(LauringUIAccountDB["RaidBuffsWhite"]) do
		if value then
			UF.RaidBuffsWhite[spellID] = true
		end
	end
...
function UF.BuffsIndicator_Filter(raidAuras, aura)
	...
	if aura.isBossAura then
		return true
	elseif Config.DB["UFs"]["AutoBuffs"] then
...
function UF:BuffsIndicator_UpdateOptions()
	...
	local disableMouse = Config.DB["UFs"]["BuffClickThrough"]
