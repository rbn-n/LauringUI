...
local function BarSetSmoothedValue(self, value)
	self._value = self:GetValue()
	activeObjects[self] = clamp(value, self._min, self._max)
end

local function BarSetSmoothedMinMaxValues(self, min, max)
...
function Core:SmoothBar(bar)
	bar._min, bar._max = bar:GetMinMaxValues()
	bar._value = bar:GetValue()

	bar.SetValue_ = bar.SetValue
	bar.SetMinMaxValues_ = bar.SetMinMaxValues
	bar.SetValue = BarSetSmoothedValue
	bar.SetMinMaxValues = BarSetSmoothedMinMaxValues
...
function Core:SetSmoothingAmount(amount)
